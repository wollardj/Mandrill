...
if Meteor.isClient
	Router.configure {
		...
		###
			Makes sure the collections that are needed across all (or at least
			most) routes are loaded before the route renders.
		###
		waitOn: ->
			user = Meteor.user()
			if not user?
		...
		###
			...
			If the route's path starts with '/admin', make sure the user is
			actually an admin, and if not, render the notFound route. If the two
			above conditions don't provoke any action, display the user's
			desired route
		###
		onBeforeAction: ->
			if Meteor.loggingIn()
...
# Attempts to locate the proper icon for the given pkg, pkginfo, or icon name.
Router.route 'munki.iconUrl', {path: '/munki/icons/:name', where: 'server'}
	.get ->
		icon = MunkiRepo.findOne {icon_name: this.params.name}
		result = ''
		if icon?
			result = icon.url()
		else
			result = Meteor.absoluteUrl 'packages/munki/images/pkg.png'
